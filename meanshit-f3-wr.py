...
df = pd.read_csv('data/f3_wr_tot_stats.csv')
# Assign RB tiers
df['TIER'] = df['FPTS/G'].apply(condition)
original_df = pd.DataFrame.copy(df)
df.drop(['Player', 'OWN', 'FL', 'G', 'TD.1', 'TD'], 1, inplace = True)
df = df[(df.TGT> 10)]
...
for i in range(n_clusters_):
    ...
    wr_cluster = temp_df[(temp_df['TIER']=='WR1')]
    wr1_rate = len(wr_cluster)/len(temp_df)
    wr1_rates[i] = wr1_rate
